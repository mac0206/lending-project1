...
const shared_1 = require("@library-system/shared");
...
class LoanService {
    ...
    async createLoan(itemId, memberId, days = 14) {
        ...
        const dueDate = (0, shared_1.calculateDueDate)(borrowDate, days);
        const loanData = {
        ...
        try {
            ...
            await catalogApi.put(`/api/items/${itemId}`, { availability: false }).catch(() => catalogApi.put(`/api/books/${itemId}`, { availability: false }));
    ...
    async getLoansByMemberId(memberId) {
        const docs = await this.loanRepository.findByMemberId(memberId);
        return docs.map(d => this.toLoan(d));
    }
    async getLoansByBookId(bookId) {
